...
$sql="SELECT * FROM `bank_db`.`customer` WHERE id=$id";
...
<?php
                        ...
                        if(isset($_REQUEST['submit2_id'])){
                            $sql_delete="DELETE FROM `bank_db`.`customer` WHERE `id` = '$delete_id'";
                            $sql_drop="DROP TABLE passbook".$delete_id;
                            mysqli_query($link, $sql_delete) or die(mysqli_error());
                            mysqli_query($link, $sql_drop) or die(mysqli_error());
